...
public class Gracz {
    ...
    public boolean dodajKarte(Karta bjKarta) {
        if(this.numKart == 10) {
    ...
    public ArrayList<Karta> pokazRekeGracza(boolean CzyNieDealer){
        System.out.println("karty gracza" + this.imie + ": " );
        for (int i = 0; i < this.numKart; i++) {
            if(i==0 && CzyNieDealer){
                System.out.println("[UKRYTA} ");
            } else {
                System.out.println(this.reka.get(i));
            }
        }
        return reka;
    }
